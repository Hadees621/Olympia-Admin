...
import { overduedata } from "../utils/utils";
...
const AllOverdueInvoices = () => {
  ...
  const handleSelectAll = () => {
    if (selectedRows.length === overduedata.length) {
      setSelectedRows([]);
    } else {
      setSelectedRows(overduedata.map((_, index) => index));
    }
  };
  ...
  return (
    <div className="w-full text-start items-center">
      ...
      <div className="flex gap-2 my-8 items-center w-[60%] justify-end mx-auto">
              ...
              <tr className="font-medium text-gray-500 uppercase text-center whitespace-nowrap">
                <th className="px-6 py-3 space-x-4 flex items-center gap-3">
                  <input
                    ...
                    checked={selectedRows.length === overduedata.length}
                  />
                  Select All
                </th>
...
export default AllOverdueInvoices;
