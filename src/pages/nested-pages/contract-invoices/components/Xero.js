...
const Xero = () => {
    ...
    const handleSelectAll = () => {
        if (selectedRows.length === xerodata.length) {
            setSelectedRows([]);
        } else {
            setSelectedRows(xerodata.map((_, index) => index));
        }
    };
                            ...
                            <tr className="text-sm font-medium text-gray-500 uppercase text-center whitespace-nowrap">
                                <th className="px-6 py-3  space-x-4 flex items-center gap-3">
                                    <input
                                        ...
                                        checked={selectedRows.length === xerodata.length}
                                    />
                                    Select All
                                </th>
                                ...
                                <th className="px-6 py-3">Xero paid</th>
                            </tr>
                        </thead>
                        <tbody>
                            {xerodata.map((row, index) => (
                                <tr
                                    key={index}
